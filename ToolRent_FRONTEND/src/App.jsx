...
const PrivateRoute = ({ element, rolesAllowed, isLoggedIn, roles, keycloak }) => {
  if (!isLoggedIn) {
    keycloak.login();
    return null;
  }
      ...
      <Box sx={{ bgcolor: '#0f172a', minHeight: '100vh' }}>
        ...
        <Box component="main">
          <Routes>
            ...
            <Route path="/inventario" element={renderPrivate(<ToolList />, ['ADMIN'])} />
            <Route path="/tools/add" element={renderPrivate(<AddEditTool />, ['ADMIN'])} />
            <Route path="/tools/edit/:id" element={renderPrivate(<AddEditTool />, ['ADMIN'])} />
            <Route path="/prestamos" element={renderPrivate(<AddLoan />, ['EMPLOYEE', 'ADMIN'])} />
            <Route path="/loans/ActiveLoanList" element={renderPrivate(<ActiveLoanList />, ['EMPLOYEE', 'ADMIN'])} />
            <Route path="/loans/list" element={renderPrivate(<LoanList />, ['ADMIN'])} />
            <Route path="/user/userRegister" element={renderPrivate(<AddEditUser />, ['EMPLOYEE', 'ADMIN'])} />
            <Route path="/users/edit/:userId" element={renderPrivate(<AddEditUser />, ['EMPLOYEE', 'ADMIN'])} />
            <Route path="/tools/decommission" element={renderPrivate(<ToolDecommission />, ['ADMIN'])} />
            <Route path="tool/movementHistory" element={renderPrivate(<KardexList />, ['ADMIN'])} />
            <Route path="/reports/lateClients" element={renderPrivate(<ReportLateClient />, ['EMPLOYEE', 'ADMIN'])} />
            <Route path="/loans/TopTools" element={renderPrivate(<ToolListRanking />, ['EMPLOYEE', 'ADMIN'])} />
            <Route path="/users/list" element={renderPrivate(<UserList />, ['EMPLOYEE', 'ADMIN'])} />
            <Route path="/loans/unpaidLoans" element={renderPrivate(<UnpaidLoansPage />, ['EMPLOYEE', 'ADMIN'])} />
