...
// Estilos y Funciones Auxiliares (Fuera del componente para evitar re-creaciones)
...
// Componente para el TextField del DatePicker (Error de nesting corregido)
const CustomDatePickerField = (params, onClick, value) => (
  <TextField
    {...params}
    sx={inputSx}
    size="small"
...
const getStatusStyles = (status) => {
  if (status === 'ATRASADO') return { bg: 'rgba(248, 113, 113, 0.1)', color: '#f87171', border: 'rgba(248, 113, 113, 0.2)' };
  if (status === 'Vigente') return { bg: 'rgba(34, 197, 94, 0.1)', color: '#4ade80', border: 'rgba(34, 197, 94, 0.2)' };
...
const ActiveLoanList = () => {
  ...
  const fetchLoansByDate = () => {
    ...
    getActiveLoansByDate(startDate.format('YYYY-MM-DD'), endDate.format('YYYY-MM-DD'))
      .then(res => setLoans(res.data))
  ...
  // Error de Optional Chaining corregido
  const handleFinePaid = (paid) => {
    if (!loanReceipt?.id) return;
    setLoading(true);
    ...
    <LocalizationProvider dateAdapter={AdapterDayjs} adapterLocale="es">
      <Box sx={{ p: 3, bgcolor: '#0f172a', minHeight: '100vh' }}>
        ...
        <Box display="flex" alignItems="center" gap={1} mb={3}>
          <Typography variant="h4" sx={{ color: "#e2e8f0", fontWeight: 700 }}>Préstamos Activos</Typography>
          <PageHelp 
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                    ...
                    <DatePicker
                      ...
                      onChange={(newValue) => {
                        ...
                        if (endDate && newValue && endDate.isBefore(newValue)) setEndDate(null);
                      }}
                      onAccept={(newValue) => {
                        setStartDate(newValue);
                      ...
                      format="DD/MM/YYYY"
                      slots={{ textField: (p) => CustomDatePickerField(p, () => setStartDateOpen(true), startDate?.format("DD/MM/YYYY") || "") }}
                    ...
                    <DatePicker
                      ...
                      format="DD/MM/YYYY"
                      slots={{ textField: (p) => CustomDatePickerField(p, () => setEndDateOpen(true), endDate?.format("DD/MM/YYYY") || "") }}
                      slotProps={{ popper: { sx: popperSx } }}
                    />
        ...
        <TableContainer component={Paper} sx={{ bgcolor: '#1e293b', borderRadius: 2, border: "1px solid rgba(148, 163, 184, 0.1)", boxShadow: "0 4px 24px rgba(0, 0, 0, 0.35)", borderTop: "3px solid rgba(56, 189, 248, 0.4)" }}>
          <Table>
            ...
            <TableBody>
              {loans.map(loan => {
                const styles = getStatusStyles(loan.loanStatus);
                return (
                  <TableRow key={loan.id} sx={{ '&:hover': { backgroundColor: 'rgba(56, 189, 248, 0.04)' }, '& td': { color: '#cbd5e1', borderBottom: '1px solid rgba(148,163,184,0.07)' } }}>
                    ...
                    <TableCell>
                      <Tooltip title="Procesar retorno físico" arrow placement="left">
                        <Button variant="contained" sx={skyButtonStyle} size="small" onClick={() => handleOpenDialog(loan)}>Devolver</Button>
                      </Tooltip>
                    </TableCell>
                  </TableRow>
                );
        ...
        {/* El resto de tus diálogos (Devolución y Boleta) se mantienen iguales */}
