  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDayjs} adapterLocale="es">
      <Box sx={{ p: 3, bgcolor: '#0f172a', minHeight: '100vh' }}>
        ...
        <Box sx={{ p: 3, mb: 3, bgcolor: '#1e293b', borderRadius: 2, border: '1px solid rgba(148, 163, 184, 0.12)', borderTop: "3px solid rgba(56, 189, 248, 0.4)", display: 'flex', flexDirection: 'column', gap: 2 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 4, flexWrap: 'wrap' }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              <Typography variant="overline" sx={{ color: '#38bdf8', fontWeight: 600 }}>Filtros</Typography>
              <DatePicker
        ...
        <TableContainer component={Paper} sx={{ bgcolor: '#1e293b', borderRadius: 2, border: "1px solid rgba(148, 163, 184, 0.12)", boxShadow: "0 4px 24px rgba(0, 0, 0, 0.35)" }}>
          <Table>
            ...
            <TableBody>
              {loans.map(loan => (
                <TableRow key={loan.id} sx={{ '&:hover': { backgroundColor: 'rgba(56, 189, 248, 0.04)' }, '& td': { color: '#cbd5e1', borderBottom: '1px solid rgba(148, 163, 184, 0.07)' } }}>
                  ...
                  <TableCell sx={{ color: '#e2e8f0', fontWeight: 500 }}>{loan.client.name}</TableCell>
                  <TableCell>{loan.client.email}</TableCell>
                  <TableCell>{loan.client.phoneNumber}</TableCell>
                  <TableCell sx={{ color: '#f87171', fontWeight: 600 }}>{loan.id}</TableCell>
                </TableRow>
              ))}
