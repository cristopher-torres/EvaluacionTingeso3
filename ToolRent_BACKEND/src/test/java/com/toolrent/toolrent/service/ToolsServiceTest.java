...
class ToolsServiceTest {
  ...
  @Mock private ToolsRepository toolsRepository;

  @Mock private UserService userService;

  @Mock private KardexService kardexService;

  @InjectMocks private ToolsService toolsService;
  ...
  void testRegisterToolInvalidName() {
    ...
    IllegalArgumentException ex =
        assertThrows(IllegalArgumentException.class, () -> toolsService.registerTool(tool, 1, rut));
  ...
  void testDecommissionToolNotFound() {
    ...
    RuntimeException ex =
        assertThrows(RuntimeException.class, () -> toolsService.decommissionTool(1L, rut));
  ...
  void testFindByIdNotFound() {
    when(toolsRepository.findById(1L)).thenReturn(Optional.empty());
    RuntimeException ex = assertThrows(RuntimeException.class, () -> toolsService.findById(1L));
    assertTrue(ex.getMessage().contains("no encontrada"));
  }
  ...
  void testGetAvailableToolNotExists() {
    when(toolsRepository.findByIdAndStatus(1L, ToolStatus.DISPONIBLE)).thenReturn(Optional.empty());

    RuntimeException ex =
        assertThrows(RuntimeException.class, () -> toolsService.getAvailableTool(1L));
  ...
  void testLoanToolNotAvailable() {
    ...
    RuntimeException ex = assertThrows(RuntimeException.class, () -> toolsService.loanTool(1L));
  ...
  void testReturnToolInvalid() {
    ...
    RuntimeException ex = assertThrows(RuntimeException.class, () -> toolsService.returnTool(1L));
